...
/// Generates `lib/app/router.dart` from `prototype/_REGISTRY/screens.csv`.
///
/// Run from `app/`:
...
class _ScreenRow {
  _ScreenRow({required this.screenId, required this.name, required this.notes});
...
List<_ScreenRow> _parseScreensCsv(String raw) {
  ...
  for (var i = 1; i < lines.length; i++) {
    ...
    final name = cols.length > 3 ? cols[3].trim() : id;
    final notes = cols.length > 8 ? cols[8].trim() : '';
    rows.add(_ScreenRow(screenId: id, name: name, notes: notes));
  }
  return rows;
}
...
String _generateRouterDart(List<_ScreenRow> rows) {
  ...
  final tombstoneIdsLiteral =
      tombstones.map((r) => "  '${r.screenId}',").join('\n');
  final tombstonePathsLiteral =
      tombstones.map((r) => "  '${r.path}',").join('\n');
